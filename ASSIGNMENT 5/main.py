...
def browse(keyword: str = Query(None), sort_by: str = Query('price'), order: str = Query('asc'), page: int = Query(1, ge=1), limit: int = Query(4, ge=1, le=20, description='Items per page')):
    result = products
    if keyword:
...
def search_by_name(customer_name: str = Query(...)):
